...
The main observation space is 3 planes of a 6x7 grid. Each plane represents a specific agent's tokens or the neutral coin, and each location in the grid represents the placement of the corresponding token. 1 indicates that the agent has a token placed in that cell, and 0 indicates they do not have a token in
...
class raw_env(AECEnv, EzPickle):
            ...
            i: spaces.Dict(
                {
                    "observation": spaces.Box(
                        low=0, high=1, shape=(6, 7, 3), dtype=np.int8
                    ),
    ...
    def observe(self, agent):
        ...
        neutral_board = np.equal(board_vals, 3)

        observation = np.stack(
            [cur_p_board, opp_p_board, neutral_board], axis=2
        ).astype(np.int8)
    ...
    def _legal_moves(self):
        # A column is legal only if the top cell is empty (not occupied by a player piece or neutral coin)
    ...
    def step(self, action):
        ...
        # assert valid move — top cell must be empty and not a neutral coin
        assert self.board[action] == 0, "played illegal move."

        piece = self.agents.index(self.agent_selection) + 1

        # Drop piece — treat both 0 (empty) as the only valid landing spot
        # Neutral coin (3) is treated as occupied so pieces stack correctly on top of it
        ...
        # check if there is a tie — board full when all cells are 1, 2, or 3
        elif all(x in [1, 2, 3] for x in self.board):
            self.terminations = {i: True for i in self.agents}
    ...
    def reset(self, seed=None, options=None):
        ...
        # Place one neutral coin randomly in the bottom row only
        # Bottom row indices are 35–41 (row 5, columns 0–6)
        rng = np.random.default_rng(seed)
        neutral_col = int(rng.integers(0, 7))
        self.board[5 * 7 + neutral_col] = 3
    ...
    def render(self):
        ...
        green_chip = get_image(os.path.join("img", "C4GreenPiece.png"))
        green_chip = pygame.transform.scale(
            green_chip, (int(tile_size * (9 / 13)), int(tile_size * (9 / 13)))
        )
